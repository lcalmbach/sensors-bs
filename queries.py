qry = {
    ...
    'map_data': """select 
	t1.station_id, t2."name" ,t2."lat", t2."long", t1."{0}", {1}(t1."value") as value 
    from 
        public."{2}" t1
        inner join public."{3}" t2 on t2.station_id = t1.station_id
    where 
        t2.lat > 40 and t1."{0}" = '{4}'
        {5}
    group by 
    ...
    'time_data_average_all': """select {0}, avg("value") as value from 
        public."{1}" t1
    ...
    'time_date_stations_raw': """select concat(t1.station_id, ' ', t2."name") as station ,t2."lat", t2."long", t1."{0}", avg(t1."value") as value 
        from public."{1}" t1 
        inner join public."{2}" t2 on t2.station_id = t1.station_id 
    ...
    'stations_all': """select *, CONCAT(station_id, ' - ' , name) as id_name from public."{}" order by station_id;""",
